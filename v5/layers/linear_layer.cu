...
LinearLayer::~LinearLayer() { cublasDestroy(handle); }
...
Matrix& LinearLayer::forward(Matrix& A) {
    ...
    // Perform matrix multiplication: Z = alpha * W * A + beta * Z
    cublasSgemm(handle, 
                CUBLAS_OP_N, 
                CUBLAS_OP_N, 
                A.shape.x, W.shape.y, W.shape.x,
                &alpha, 
                A.data_device.get(), A.shape.x,
                W.data_device.get(), W.shape.x,
                &beta, 
                Z.data_device.get(), Z_shape.x);

    // Add bias
    // Note: Assuming b is a Vector with length W.shape.y
    for (int i = 0; i < Z.shape.y; ++i) {
        cublasSaxpy(handle, Z.shape.x, &alpha, b.data_device.get() + i, 0, Z.data_device.get() + i * Z.shape.x, 1);
    }
